        ...
        try:
            while True:
                ...
                if not _encrypted_message:
                    break
                print(f"Received Encrypted Message: {_encrypted_message}")
                _decrypted_message = _decrypt_data(_encrypted_message)
                print(f"Decrypted Message: {_decrypted_message}")
                _response = f"Server received: {_decrypted_message}"
                _encrypted_response = _encrypt_data(_response)
                print(f"Sending Encrypted Response: {_encrypted_response}")
